...
def plot_total_cost(
    ...
    total_costs = [generation[0] for generation in generations_best_costs]

    generations = range(len(total_costs))

    plt.figure(figsize=(10, 6))

    plt.plot(generations, total_costs, label="Total Cost")
...
def plot_cost_components(
    ...
    skill_costs = [generation[1][0] for generation in generations_best_costs]

    overtime_costs = [generation[1][1] for generation in generations_best_costs]

    imbalance_costs = [generation[1][2] for generation in generations_best_costs]

    proficiency_costs = [generation[1][3] for generation in generations_best_costs]

    priority_costs = [generation[1][4] for generation in generations_best_costs]

    generations = range(len(generations_best_costs))

    plt.figure(figsize=(12, 7))

    plt.plot(generations, skill_costs, label="Skill Mismatch")

    plt.plot(generations, overtime_costs, label="Overtime")

    plt.plot(generations, imbalance_costs, label="Workload Imbalance")

    plt.plot(generations, proficiency_costs, label="Proficiency")

    plt.plot(generations, priority_costs, label="Priority Imbalance")
...
def plot_all_costs(
    ...
    plt.plot(generations, total_costs, label="Total Cost", linewidth=2)
...
def plot_employee_workload(
    ...
    workloads = {employee_id: 0 for employee_id in range(employees_num)}

    for task_index, employee_index in enumerate(best_chromosome):
        workloads[employee_index] += tasks_data[task_index]["hours"]

    employee_names = [
        employees_data[employee_id]["name"] for employee_id in range(employees_num)
    ]

    assigned_hours = [workloads[employee_id] for employee_id in range(employees_num)]
    ...
    plt.bar(positions, assigned_hours, label="Assigned Hours")

    plt.plot(positions, available_hours, marker="o", label="Available Hours")

    plt.xticks(positions, employee_names, rotation=45, ha="right")
...
def plot_employee_priority_load(
    ...
    priority_loads = {employee_id: 0 for employee_id in range(employees_num)}

    for task_index, employee_index in enumerate(best_chromosome):
        task = tasks_data[task_index]

        priority_loads[employee_index] += task["priority"] * task["hours"]

    employee_names = [
        employees_data[employee_id]["name"] for employee_id in range(employees_num)
    ]

    loads = [priority_loads[employee_id] for employee_id in range(employees_num)]
    ...
    plt.bar(positions, loads, label="Priority Load")
